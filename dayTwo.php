...
<html>
	...
	<script type="text/javascript">
		var p                   = document.getElementById('algo');
		var str                 = p.innerText;
		str                     = str.trim();
		...
		for (var i = 0; i<strSplit.length; i++)
		{
			...
			//prism surface plus extra paper (singular because it's a simple variable)
			surface_ExtraPaper = surfaceArea(prism) + smallestSide(prism);
		...
		function smallestSide(lwh)
		{
			...
			for(var i = 0; i < lwh.length; i++)
				for(var j = 0; j< lwh.length; j++)
				{
					if( i == j)
						continue;
					var result = parseInt(lwh[i]) * parseInt( lwh[j] ) ;
					results.push(result);
				}
			//return results;
			...
			for(var i = 0; i<results.length; i++ )
			{
				for(var j = 0; j< results.length; j++ )
				{
					if( i == j)
						continue;
					if(typeof smallestSide == 'undefined')
						smallestSide = results[0] ;
					var aux = results[i];
					if(smallestSide > aux )
						smallestSide = aux;
				}
